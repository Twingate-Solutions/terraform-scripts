...
# Search for a specific existing group
...
# (Optional) Pull in an existing group from id
#data "twingate_group" "tf_demo_eng" {
#    id                          = var.tg_eng_group
#}

# (Optional) Pull in an existing group from filter
...
# (Optional) Pull in an existing group from filter
#data "twingate_groups" "tf_demo_sales" {
#    name                        = "Sales"
#}
...
# Create a demo group and add users to the group
resource "twingate_group" "tf_demo_vendors" {
    name                        = "TF Demo - Vendors"
    user_ids                    = var.tg_users
}
...
resource "twingate_resource" "tf_demo_aws_resource" {
    ...
    #address                     = aws_instance.private_resource.private_ip # IP Address
    address                     = "tg-tf-demo.int" # FQDN
    remote_network_id           = twingate_remote_network.tf_demo_aws_network.id
    ...
    dynamic "access_group" {
        for_each = toset([                                                                                    # Loop over each group id
            twingate_group.tf_demo_devops.id, 
            twingate_group.tf_demo_vendors.id, 
            #data.twingate_group.tf_demo_eng.id
            ])              
        content {
    ...
    # (Optional) Adding multiple groups from twingate_groups data sources
    #dynamic "access_group" {
    #    for_each = setunion(
    ...
    #        toset([data.twingate_group.tf_demo_sales.id])
    #    )
    #    content {
    #        group_id                           = access_group.value                                           # Assign groups to resource
    #        security_policy_id                 = data.twingate_security_policy.tf_demo_trusted-30day-nomfa.id # Override default resource policy on groups
